...
def Read_Data(conf,year):
    # open file
    f = open('../data_files/CFB/'+str(conf)+'/'+str(year)+'games.txt')
    # read all lines
...
def Main():
    ...
    for year in range(1995,2004):
        f1.write(',%d,,,\n' % year)
        f2.write(',%d' % year)
        adj, elo_rating, forw_pred, rnd = Read_Data('Atlantic Coast',year)
        numRounds = len(rnd)
        for k in range(numRounds):
    ...
    for year in range(1995,2013):
        f1.write(',%d,,,\n' % year)
        f2.write(',%d' % year)
        adj, elo_rating, forw_pred, rnd = Read_Data('Big East',year)
        numRounds = len(rnd)
        for k in range(numRounds):
            f1.write(',,%d,%.4f,%.4f\n' % (k+1,specR(adj[k]),forw_pred[k]/float(rnd[k][1]-rnd[k][0]+1)))
        # year summary
    ...
    for year in range(1999,2012):
        f1.write(',%d,,,\n' % year)
        f2.write(',%d' % year)
        adj, elo_rating, forw_pred, rnd = Read_Data('Mountain West',year)
        numRounds = len(rnd)
        for k in range(numRounds):
            f1.write(',,%d,%.4f,%.4f\n' % (k+1,specR(adj[k]),forw_pred[k]/float(rnd[k][1]-rnd[k][0]+1)))
        # year summary
