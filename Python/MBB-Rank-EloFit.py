# MBB-Rank-EloFit: Rankability Measures for MBB conferences and Comparison to the Elo ranking fit
#
...
def Main():
    ...
    print('America East: ')
    x = []; y = []; z = []
    f1.write('America East,Year,Round,Rank,Forw-Pred\n')
    f2.write('America East,Year,Rank,Forw-Pred,Back-Pred\n')
    for year in range(2002,2020):
        f1.write(',%d,,,\n' % year)
        f2.write(',%d' % year)
        adj, elo_rating, forw_pred, rnd = Read_Data('America East',year)
        numRounds = len(rnd)
        for k in range(numRounds):
    ...
    print('Big West: ')
    x = []; y = []; z = []
    f1.write('Big West,Year,Round,Rank,Forw-Pred\n')
    f2.write('Big West,Year,Rank,Forw-Pred,Back-Pred\n')
    for year in range(2002,2020):
        f1.write(',%d,,,\n' % year)
        f2.write(',%d' % year)
        adj, elo_rating, forw_pred, rnd = Read_Data('Big West',year)
        numRounds = len(rnd)
        for k in range(numRounds):
            f1.write(',,%d,%.4f,%.4f\n' % (k+1,specR(adj[k]),forw_pred[k]/float(rnd[k][1]-rnd[k][0]+1)))
        # year summary
    ...
    print('Ivy League: ')
    x = []; y = []; z = []
    f1.write('Ivy League,Year,Round,Rank,Forw-Pred\n')
    f2.write('Ivy League,Year,Rank,Forw-Pred,Back-Pred\n')
    for year in range(2002,2020):
        f1.write(',%d,,,\n' % year)
        f2.write(',%d' % year)
        adj, elo_rating, forw_pred, rnd = Read_Data('Ivy League',year)
        numRounds = len(rnd)
        for k in range(numRounds):
            f1.write(',,%d,%.4f,%.4f\n' % (k+1,specR(adj[k]),forw_pred[k]/float(rnd[k][1]-rnd[k][0]+1)))
        # year summary
    ...
    x = []; y = []; z = []
    f1.write('Patriot League,Year,Round,Rank,Forw-Pred\n')
    f2.write('Patriot League,Year,Rank,Forw-Pred,Back-Pred\n')
    for year in range(2002,2020):
        f1.write(',%d,,,\n' % year)
        f2.write(',%d' % year)
        adj, elo_rating, forw_pred, rnd = Read_Data('Patriot League',year)
        numRounds = len(rnd)
        for k in range(numRounds):
            f1.write(',,%d,%.4f,%.4f\n' % (k+1,specR(adj[k]),forw_pred[k]/float(rnd[k][1]-rnd[k][0]+1)))
        # year summary
    ...
    print('West Coast: ')
    x = []; y = []; z = []
    f1.write('West Coast,Year,Round,Rank,Forw-Pred\n')
    f2.write('West Coast,Year,Rank,Forw-Pred,Back-Pred\n')
    for year in range(2002,2019):
        f1.write(',%d,,,\n' % year)
        f2.write(',%d' % year)
        adj, elo_rating, forw_pred, rnd = Read_Data('West Coast',year)
        numRounds = len(rnd)
        for k in range(numRounds):
            f1.write(',,%d,%.4f,%.4f\n' % (k+1,specR(adj[k]),forw_pred[k]/float(rnd[k][1]-rnd[k][0]+1)))
        # year summary
