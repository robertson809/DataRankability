...
def Read_Data(year):
    ...
    for k in range(numRounds):
        ...
        #            forw_pred[k] = forw_pred[k] + 1
            ...
            if(Ranking_Fit(x,elo_rating[k-1]) <= np.finfo(float).eps):
                forw_pred[k] = forw_pred[k] + 1
...
def Back_Pred(year,elo_rating):
    ...
    #            back_pred = back_pred + 1
            ...
            x[j] = x[j] + K*(u-0.5)
        # record predictability
        if(Ranking_Fit(x,elo_rating) <= np.finfo(float).eps):
            back_pred = back_pred + 1
    # close file
    f.close()
    # return
...
def Main():
    ...
    for year in range(2013,2019):
        ...
        print(sum(forw_pred[1:numRounds]))
        print(((numRounds-1)*(numPlayers//2)))
        y.append(sum(forw_pred[1:numRounds])/((numRounds-1)*(numPlayers/2.)))
