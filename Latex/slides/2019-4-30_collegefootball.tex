...
\vfill
\begin{algorithmic}
\STATE{$\text{function}~[r] = \text{rankS}(A):$}
\bindent
\STATE{$n = \text{size}(A)$}
...
\vfill
\begin{algorithmic}
\STATE{$\text{function}~[r] = \text{rankA}(A):$}
\bindent
\STATE{$n = \text{size}(A)$}
\STATE{Let $\mathcal{L}$ be laplacian of dominance graph of size $n$}
\STATE{Let $L$ be laplacian of the graph associated with $A$}.
